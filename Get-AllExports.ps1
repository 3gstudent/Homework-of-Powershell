...
.EXAMPLE
C:\PS> $Path = "C:\Windows"
C:\PS> ForEach($file in (Get-ChildItem -recurse -Filter "*.dll" -Path $Path  -ErrorAction SilentlyContinue )) 
C:\PS> {
C:\PS>     Get-Exports -DllPath $file.PSPath.Substring($file.PSPath.IndexOf(":")+2)
C:\PS> }
...
#		$ExportArray += $Object
		...
		if ($FunctionName -match "minidump")
		{
			echo "[+] $($DllPath)-->$($FunctionName)"
		}
	}
